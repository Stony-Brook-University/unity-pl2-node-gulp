...
gulp.task('pl-copy:jsheader', function () {
  return gulp.src('**/*.js', {cwd: resolvePath(paths().source.jsheader)})
    .pipe(concat('header.dev.js'))
    .pipe(gulp.dest(resolvePath(paths().public.jsheader)))
    .pipe(rename('header.min.js'))
    .pipe(uglify())
    .pipe(gulp.dest(resolvePath(paths().public.jsheader)));
});

gulp.task('pl-copy:jsfooter', function () {
  return gulp.src('**/*.js', {cwd: resolvePath(paths().source.jsfooter)})
    .pipe(concat('footer.dev.js'))
    .pipe(gulp.dest(resolvePath(paths().public.jsfooter)))
...
// placeholder content is specific to styleguide and shouldn't be included in final compiled assets for distribution.
// they are typically images, video's or other resources that are meant to simulate actual content but aren't referenced in any stylesheets or code.
gulp.task('pl-copy:placeholders', function () {
  return gulp.src(resolvePath(paths().source.placeholders) + '/**/*')
    .pipe(gulp.dest(resolvePath(paths().public.placeholders)))
    .pipe(browserSync.stream());
});
